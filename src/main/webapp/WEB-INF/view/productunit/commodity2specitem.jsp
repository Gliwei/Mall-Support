...
$(function() {
	...
	$('#optgroup').multiSelect({
		...
		afterSelect: function(val){
			// 选择
			$.post("${rootPath}commodity/saveC2S","id="+cid+"&itemIds="+val,function(result){
				// alert(result);
			});
		},
		afterDeselect: function(val){
			// 取消选择
			$.post("${rootPath}commodity/delC2S","id="+cid+"&itemIds="+val,function(result){
				// alert(result);
			});
		}
		});
								...
								<div class="controls">
									<select id='optgroup' multiple='multiple'>
									  ...
									  <optgroup label='${spec.name}'>
									    <c:forEach items="${spec.specitems}" var="item" >
									    <option value='${item.id}' 
									    <c:forEach items="${entity.commodity2specitems}" var="c2s" >
									     	<c:if test="${c2s.specitem.id == item.id}"> selected </c:if>
									    </c:forEach>
